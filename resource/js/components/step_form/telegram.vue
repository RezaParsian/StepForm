        ...
        <div class="row mx-auto">
            <div class="col-md-7">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" v-model="more" rows="3" name="detail" id="detail"></textarea>
                </div>
            </div>
...
export default {
    ...
    data() {
        return {
            file: "",
            post_text: this.state.telegram_post_text,
            post_link: this.state.telegram_post_link,
            more: this.state.telegram_more
        }
    },
    watch: {
        post_text(val) {
            Bus.$emit("state", "telegram_post_text", val);
        },
        post_link(val) {
            Bus.$emit("state", "telegram_post_link", val);
        },
        more(val) {
            Bus.$emit("state", "telegram_more", val);
        }
    },
    methods: {
        checkData() {
